	...
	def __init__(__self__, __name__="", **__params__):
		...
		class __sync__(__self__.__thread__.Thread):
			def run(self, __self__=__self__, __name__=__name__, __params__=__params__):
				...
				__env__ = locals()
				if not __isdir__ and "only" in __params__: __oldenv__ = dict(__env__)
				__env__.pop("__self__")
				exec(__script__, __env__)
				if not __isdir__ and "only" in __params__:
					__newenv__ = __env__
					__env__ = __oldenv__
					for __req__ in __params__["only"]: __env__.update({__req__: __newenv__[__req__]})
					...
					if "shared" not in __params__ or __params__["shared"] == "time":
						for __item__ in __items__:
							...
							if "only" not in __params__ or __item__ in __params__["only"]:
								try:
									if __source__ == "local": __child__ = use(__item__, path=__dir__+__name__, level="private", family=__children__)
									elif __source__ == "remote": __child__ = use(__item__, url=__join__(__url__+"/", __item__+".py"), level="private", family=__children__)
						...
						class __createchild__(__self__.__thread__.Thread):
							...
							def run(self):
								try: 
									if __source__ == "local": __childdict__[self.item] = use(self.item, path=__dir__+__name__, level="private", family=__children__)
									elif __source__ == "remote": __childdict__[self.item] = use(self.item, url=__join__(__url__+"/", self.item+".py"), level="private", family=__children__)
